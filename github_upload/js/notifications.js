    ...
    supabaseClient
      ...
      .on(
        ...
        (payload) => {
          ...
          showNotificationModal(payload.new.title, payload.new.message);
          loadNotifications();
        }
      )
      .subscribe();
...
function showNotificationModal(title, message) {
  const existing = document.getElementById('rt-notification-modal');
  if (existing) existing.remove();

  const modal = document.createElement('div');
  modal.id = 'rt-notification-modal';
  modal.style.cssText = `
    position: fixed; top: 20px; left: 50%; transform: translateX(-50%);
    width: 90%; max-width: 380px; background: #121620;
    border: 1px solid #00e676; box-shadow: 0 10px 30px rgba(0,0,0,0.7);
    border-radius: 14px; padding: 16px; z-index: 10000; color: #fff;
    font-family: system-ui, -apple-system, sans-serif;
  `;
  modal.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
      <span style="font-weight:700; font-size:15px; color:#00e676; display:flex; align-items:center; gap:6px;">
        🔔 ${title}
      </span>
      <button onclick="document.getElementById('rt-notification-modal').remove()" style="background:none; border:none; color:#888; font-size:18px; cursor:pointer; padding:0 4px;">✕</button>
    </div>
    <p style="margin:0; font-size:13px; color:#d0d0d0; line-height:1.4;">${message}</p>
  `;
