...
-- 5b. P2P PAYOUT QUEUE TABLE
CREATE TABLE IF NOT EXISTS public.p2p_payout_queue (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
    ...
    associated_submission_id UUID REFERENCES public.task_submissions(id) ON DELETE SET NULL,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    completed_at TIMESTAMPTZ
);

-- Link p2p_queue_id back to task_submissions
ALTER TABLE public.task_submissions 
ADD COLUMN IF NOT EXISTS p2p_queue_id UUID REFERENCES public.p2p_payout_queue(id) ON DELETE SET NULL;
...
BEGIN
    ...
    -- If this submission has a p2p_queue_id, complete the P2P payout!
    IF v_sub.p2p_queue_id IS NOT NULL THEN
        PERFORM public.complete_p2p_payout(v_sub.p2p_queue_id, p_submission_id);
    END IF;
...
-- ====================================================
-- P2P PAYOUT QUEUE TRIGGERS & PROCEDURES
-- ====================================================

-- 11. acquire_p2p_payout function
CREATE OR REPLACE FUNCTION public.acquire_p2p_payout(
...
RETURNS TABLE (
    id UUID,
    phone_number TEXT,
    lock_expires_in_seconds INTEGER
) AS $$
DECLARE
    v_user_id UUID;
    v_queue_id UUID;
    v_phone_number TEXT;
    v_lock_seconds INTEGER := 120;
BEGIN
    v_user_id := auth.uid();
    IF v_user_id IS NULL THEN
        RAISE EXCEPTION 'Not authenticated';
    ...
    -- Clean up any expired locks inline
    UPDATE public.p2p_payout_queue
    SET status = 'available',
        locked_by = NULL,
        lock_expires_at = NULL
    WHERE status = 'locked' AND lock_expires_at < NOW();

    -- Select the oldest available payout (excluding own)
    SELECT q.id, q.phone_number INTO v_queue_id, v_phone_number
    FROM public.p2p_payout_queue q
    WHERE q.payment_method = p_payment_method
    ...
    IF v_queue_id IS NOT NULL THEN
        ...
        SET status = 'locked',
            locked_by = v_user_id,
            lock_expires_at = NOW() + (v_lock_seconds || ' seconds')::INTERVAL
        WHERE public.p2p_payout_queue.id = v_queue_id;

        RETURN QUERY SELECT v_queue_id, v_phone_number, v_lock_seconds;
    ELSE
        RETURN;
    END IF;
END;
...
-- 12. add_to_p2p_queue function
CREATE OR REPLACE FUNCTION public.add_to_p2p_queue(
    p_user_phone TEXT,
    p_payment_method TEXT,
    p_amount NUMERIC,
    p_count INTEGER,
    p_associated_submission_id UUID DEFAULT NULL
)
RETURNS BOOLEAN AS $$
DECLARE
    v_user_id UUID;
...
BEGIN
    ...
    SELECT id INTO v_user_id FROM public.profiles WHERE phone = p_user_phone LIMIT 1;
    IF v_user_id IS NULL THEN
        v_user_id := auth.uid();
    END IF;

    FOR i IN 1..p_count LOOP
        INSERT INTO public.p2p_payout_queue (user_id, phone_number, payment_method, amount, status, associated_submission_id)
        VALUES (v_user_id, p_user_phone, p_payment_method, p_amount, 'available', p_associated_submission_id);
    END LOOP;

    -- If we are adding an associated submission, update its status so admin knows it is in the queue
    IF p_associated_submission_id IS NOT NULL THEN
        UPDATE public.task_submissions
        SET admin_note = 'Added to P2P Payout Queue'
        WHERE id = p_associated_submission_id;
    END IF;
...
-- 13. complete_p2p_payout function
CREATE OR REPLACE FUNCTION public.complete_p2p_payout(
    p_queue_id UUID,
    p_by_submission_id UUID
)
RETURNS BOOLEAN AS $$
DECLARE
    v_queue public.p2p_payout_queue%ROWTYPE;
...
BEGIN
    ...
    SELECT * INTO v_queue FROM public.p2p_payout_queue WHERE id = p_queue_id;
    IF NOT FOUND OR v_queue.status = 'completed' THEN
        RETURN FALSE;
    END IF;
    ...
    -- If there is an associated submission for the queue owner (User A)
    IF v_queue.associated_submission_id IS NOT NULL THEN
        SELECT * INTO v_sub FROM public.task_submissions WHERE id = v_queue.associated_submission_id;
        IF FOUND AND v_sub.status = 'refund_pending' THEN
            SELECT * INTO v_task FROM public.tasks WHERE id = v_sub.task_id;
            
            -- 1. Create refund record
            INSERT INTO public.refunds (user_id, task_submission_id, amount, refund_number, status, admin_note, refunded_at)
            VALUES (v_sub.user_id, v_queue.associated_submission_id, v_sub.amount, v_queue.phone_number, 'refunded', 'P2P Payout completed by peer submission ID: ' || p_by_submission_id, NOW())
            RETURNING id INTO v_refund_id;

            -- 2. Update submission status to 'refunded'
            UPDATE public.task_submissions
            SET status = 'refunded',
                completed_at = NOW(),
                updated_at = NOW()
            WHERE id = v_queue.associated_submission_id;

            -- 3. Credit task bonus to User A's wallet
            INSERT INTO public.wallet_transactions (user_id, type, amount, reference_type, reference_id, description)
            VALUES (
                v_sub.user_id,
                'bonus',
                v_task.bonus_amount,
                'refund',
                v_queue.associated_submission_id,
                'Cashback bonus for completed task (P2P): ' || v_task.title
            );
            ...
            VALUES (
                v_sub.user_id,
                'টাস্ক পেমেন্ট রিফান্ড সম্পন্ন! ৳' || v_sub.amount,
                'আপনার ' || v_task.title || ' টাস্কের ৳' || v_sub.amount || ' রিফান্ড সফলভাবে সম্পন্ন হয়েছে এবং ৳' || v_task.bonus_amount || ' বোনাস ওয়ালেটে যোগ হয়েছে।'
            );

            -- 5. Handle referral commission milestone logic for User A
            SELECT referred_by INTO v_referrer_id FROM public.profiles WHERE id = v_sub.user_id;
            IF v_referrer_id IS NOT NULL THEN
                SELECT COUNT(*) INTO v_completed_count
                FROM public.task_submissions
                    ...
                    IF v_earned_commission < 20000.00 THEN
                        ...
                        IF v_bonus > 0 THEN
                            ...
                            IF NOT EXISTS (
                                ...
                                INSERT INTO public.wallet_transactions (user_id, type, amount, reference_type, reference_id, description)
                                VALUES (v_referrer_id, 'bonus', v_bonus, 'refund', v_queue.associated_submission_id, v_description);
...
-- P2P Policy rules
ALTER TABLE public.p2p_payout_queue ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow P2P read access for authenticated users" ON public.p2p_payout_queue FOR SELECT TO authenticated USING (TRUE);
CREATE POLICY "Allow P2P admin management" ON public.p2p_payout_queue FOR ALL TO authenticated USING (public.is_admin(auth.uid()));

-- 14. release_p2p_payout function
CREATE OR REPLACE FUNCTION public.release_p2p_payout(
    p_queue_id UUID
)
RETURNS BOOLEAN AS $$
BEGIN
