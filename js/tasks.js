...
function startTask(taskId) {
  ...
  content.innerHTML = `
    ...
    <div style="background:${brandGrad}; border-radius:12px 12px 0 0; padding:14px 20px; margin:-20px -20px 20px -20px; display:flex; align-items:center; gap:12px;">
      ...
      <div>
        ...
        <div style="font-size:11px;color:rgba(255,255,255,0.75);">নিরাপদ ও তাৎক্ষণিক রিফান্ড</div>
      </div>
    </div>
    ...
    <div style="background:rgba(255,255,255,0.03); border:1px solid var(--border-color); border-radius:12px; padding:16px; margin-bottom:14px;">
      ...
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
        <div>
          ...
          <p style="font-size:22px;font-weight:800;color:#fff;letter-spacing:1px;font-family:monospace;">${task.payment_number}</p>
        </div>
        <button id="copy-num-btn" type="button" onclick="copyToClipboard('${task.payment_number}', this)"
    ...
    <!-- hidden fields -->
    <input type="hidden" id="submit-task-id" value="${task.id}">
    <input type="hidden" id="submit-amount" value="${task.payment_amount}">
  `;
  ...
  // Submit button
  document.getElementById('btn-submit-task').addEventListener('click', () => submitTask(task));
...
async function submitTask(task) {
  ...
  try {
    ...
    await supabaseClient.from('payments').insert({
      ...
      sender_number: '',
      receiver_number: task.payment_number,
      amount,
      transaction_id: txnId,
